...
public class EntradaDAOMySQL implements iEntradaDAO {
    ...
    public List<iEntrada> searchByEvento(int idEvento) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error buscando entradas por evento: " + e.getMessage());
        }
        return lista;
    }
     ...
     * @param entrada Objeto con los datos de la entrada (precio, cantidad, info).
     * @param idEvento ID del evento al que pertenece esta entrada.
    ...
    public boolean registerEntrada(iEntrada entrada, int idEvento) {
        String sql = """
            INSERT INTO entrada 
            (ID_EVENTO, Precio, Cantidad, Informacion) 
            VALUES (?, ?, ?, ?)
            """;
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error registrando la entrada: " + e.getMessage());
            return false;
        }
    }

    /**
     * Metodo auxiliar para recuperar el ID de una entrada basándose en sus datos.
     * @param entrada Objeto entrada.
     * @param idEvento ID del evento asociado.
     * @return ID numérico de la entrada o -1 si no existe.
     */
    @Override
    public int getID(iEntrada entrada, int idEvento) {
        String sql = """
            SELECT ID_ENTRADA 
            FROM entrada 
            WHERE ID_EVENTO = ? AND Precio = ? AND Informacion = ?
            """;
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error obteniendo el ID: " + e.getMessage());
        }
        return -1; 
    }

    /**
     * Metodo para actualizar los datos de una entrada existente.
     * @param id Identificador unico de la entrada a modificar.
    ...
    public boolean updateEntrada(int id, iEntrada entrada) {
        String sql = """
            UPDATE entrada 
            SET Precio = ?, Cantidad = ?, Informacion = ? 
            WHERE ID_ENTRADA = ?
            """;
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error actualizando la entrada: " + e.getMessage());
            return false;
        }
    }
    ...
    public boolean deleteEntrada(int id) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Error eliminando la entrada: " + e.getMessage());
            return false;
        }
    }
}